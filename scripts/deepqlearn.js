...
(function(global) {
  ...
  var Brain = function(num_states, num_actions, opt) {
    ...
    // what epsilon value do we want to start at? 1.0 => purely random policy to start
    ...
    this.epsilon = this.epsilon_max; // controls exploration exploitation tradeoff. Should be annealed over time
  ...
  Brain.prototype = {
    ...
    forward: function(input_array) {
      ...
      if(this.forward_passes > this.temporal_window) {
        ...
        if(this.learning) {
          ...
          this.epsilon = Math.min(this.epsilon_max, Math.max(this.epsilon_min, this.epsilon_max-(this.age - this.learning_steps_burnin)/(this.learning_steps_total - this.learning_steps_burnin))); 
        } else {
    ...
    backward: function(reward, terminal) {
      ...
      //If we've reached the end of an episode, we should reset this count, so that
      // we build up a new set of experience transitions, rather than thinking that
      // a new game is a continuation of the current one.
      if(terminal) { 
